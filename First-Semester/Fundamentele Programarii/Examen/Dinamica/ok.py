...
def cea_mai_lunga_secventa_cu_nr_pare_descrescatoare(lst):
    ...
    for i in range(n):
        if lst[i] % 2 == 1:
            s.append(1)  # Orice element impar este in sine o secventa descrescatoare de lungime 1
        else:
            s.append(0)  # Nu exista nici o secventa de nr impare care sa se termine intr-un numar par

    for i in range(n):
        for j in range(i):
            if lst[i] % 2 == 1 and lst[j] % 2 == 1 and lst[j] >= lst[i] and s[j] + 1 > s[
                i]:  # daca elementele de pe pozitia i si de pe pozitia j sunt compatibile, si secventa obtiunuta e mai mare decat cea de pana acum
                s[i] = s[j] + 1
                p[i] = j
